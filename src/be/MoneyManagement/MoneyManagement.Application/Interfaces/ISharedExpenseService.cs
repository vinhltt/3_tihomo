...
public interface ISharedExpenseService
{
    ...
    Task<SharedExpenseParticipantResponseDto> UpdateParticipantAsync(Guid id, UpdateSharedExpenseParticipantRequestDto updateDto);
    ...
    /// <returns>Updated participant response DTO (EN)<br/>DTO phản hồi người tham gia đã cập nhật (VI)</returns>
    Task<SharedExpenseParticipantResponseDto> RecordPaymentAsync(Guid participantId, decimal amount, string? paymentMethod = null);
