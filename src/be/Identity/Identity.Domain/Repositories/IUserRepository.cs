...
public interface IUserRepository : IBaseRepository<User, Guid>
{
    ...
    Task<bool> IsUsernameExistsAsync(string username, Guid excludeUserId, CancellationToken cancellationToken = default);
    Task UpdateLastLoginAsync(Guid userId, DateTime lastLoginAt, CancellationToken cancellationToken = default);
    Task<IEnumerable<User>> GetPagedAsync(int page, int pageSize, string? search = null, CancellationToken cancellationToken = default);
