    ...
    : ControllerBase
{
    ...
    public async Task<ActionResult<Identity.Api.Models.LoginResponse>> SocialLogin([FromBody] SocialLoginRequest request)
    {
        try
        {
            ...
            var response = new Identity.Api.Models.LoginResponse
            {
                User = userInfo,
    ...
    /// </summary>    [HttpPost("refresh-token")]
    public async Task<ActionResult<Identity.Contracts.Authentication.RefreshTokenResponse>> RefreshToken([FromBody] Identity.Contracts.Authentication.RefreshTokenRequest request)
    {
        try
        {
            ...
            {
                return BadRequest("Failed to rotate refresh token");
            }            // Generate new access token using JWT service
            // Tạo access token mới bằng JWT service
            var newAccessToken = jwtService.GenerateAccessToken(user);            var response = new Identity.Contracts.Authentication.RefreshTokenResponse(
                AccessToken: newAccessToken,
                RefreshToken: newRefreshToken,
                ExpiresAt: DateTime.UtcNow.AddHours(1) // 1 hour expiration for access token
            );
    ...
    public async Task<ActionResult> Logout([FromBody] LogoutRequest request)
    {
        try
        {
            ...
            if (!revokeSuccess)
            {
                logger.LogWarning("Failed to revoke refresh token during logout");
