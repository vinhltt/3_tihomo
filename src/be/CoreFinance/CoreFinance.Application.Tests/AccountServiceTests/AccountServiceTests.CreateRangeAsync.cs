...
public partial class AccountServiceTests
{
    ...
    /// </summary>
    [Fact]    public async Task CreateAsync_List_RepositoryReturnsZeroAffected_ThrowsNullReferenceException()
    {
        // Arrange
        var numberOfAccounts = 5;
        var createRequests = new Faker<AccountCreateRequest>()
    ...
    ///     trình tạo. (VI)
    /// </summary>
    [Fact]    public async Task CreateAsync_List_RepositoryThrowsException_PropagatesException()
    {
        // Arrange
        var numberOfAccounts = 5;
        var createRequests = new Faker<AccountCreateRequest>()
            ...
            .RuleFor(r => r.Currency, f => f.Finance.Currency().Code)
            .RuleFor(r => r.UserId, Guid.NewGuid())
            .Generate(numberOfAccounts);var repoMock = new Mock<IBaseRepository<Account, Guid>>();
        repoMock.Setup(r => r.CreateAsync(It.IsAny<List<Account>>()))
